...
def get_args_parser():
    ...
    parser.add_argument("--use_coop", action='store_true', help='use coop')
    parser.add_argument('--amp', action='store_true',
                        help="Train with mixed precision")
    parser.add_argument('--use_moe_lora', action='store_true',
                        help="Train with moe lora")
    parser.add_argument("--use_adapter", action='store_true', help='use adapter')
    parser.add_argument("--use_prompt", action='store_true', help='use adapter')
    parser.add_argument("--use_zira", action='store_true', help='use adapter')
    return parser
...
def main(args):
        ...
        for name, parameter in model.named_parameters():
            for keyword in args.freeze_keywords:
                if keyword in name:
                    ...
                    break
            if args.use_moe_lora and name.startswith("module.transformer.encoder.fusion_layers.") and 'lora' not in args.freeze_keywords:
                train_key_word_list = ['lora_A', 'lora_B']
                for train_key_word in train_key_word_list:
                    if train_key_word in name:
                        parameter.requires_grad_(True)
            if args.use_adapter and name.startswith("module.backbone.") and "adapter" in name:
                parameter.requires_grad_(True)
    ...
    if (not args.resume) and args.pretrain_model_path:
        ...
        if args.use_moe_lora:
            ...
            new_tmp_st = copy.deepcopy(_tmp_st)
            
            for key in _tmp_st:
                if key.startswith("transformer.encoder.fusion_layers."):
                    if ".v_proj.weight" in key or '.l_proj.weight' in key or ".v_proj.bias" in key or ".l_proj.bias" in key \
                            or ".values_v_proj.weight" in key or ".values_v_proj.bias" in key or ".values_l_proj.weight" in key or ".values_l_proj.bias" in key:
                        key_split = key.split(".")
                        new_key = ".".join(key_split[:-1])+".base_layer_."+key_split[-1]
                        # print(new_key)
                        new_tmp_st[new_key] = _tmp_st[key]
                    # else:
            _tmp_st = new_tmp_st
