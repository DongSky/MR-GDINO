...
#     def __init__(self, length=5, embed_dim=96, embed_dim_key=1536, embedding_key='mean', prompt_init='uniform', prompt_pool=True, 
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.use_adapter:
            self.adapter = Adapter(hidden_dim=dim)
    ...
    def forward(self, x, mask_matrix):
        ...
        # FFN
        if self.use_adapter:
            x = shortcut + self.drop_path(self.adapter(x))
        else:
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        for blk in self.blocks:
            blk.H, blk.W = H, W
            if self.use_checkpoint and cnt>0:
